...
面向求职者的 AI 模拟面试工具，支持语音 / 文字作答、AI 自适应追问和多维评分报告。当前覆盖 8 个预设岗位，并支持研究生复试、工作答辩等任意场景的自定义面试。
...
- **任意场景自定义**：粘贴或上传 PDF/DOCX 题库与个人材料，AI 以题库为主选题，并结合材料动态追问。
...
4. 重新部署后，从页面右上角进入「反馈」并提交一条测试反馈，确认飞书群收到消息。
...
- 首页：选择预设岗位或自定义面试，配置模式、职级、轮次和题量；自定义面试支持粘贴或上传 PDF/DOCX 题库与个人材料
...
评分维度按岗位区分：产品岗侧重逻辑、产品判断与表达；AI 技术岗侧重技术深度、系统设计和工程实践；前端岗增加前端基础、架构、性能与质量；Java 后端岗增加 Java 基础、系统设计、数据与中间件；数据分析岗侧重 SQL、统计实验、业务洞察和可视化；运营岗侧重增长策略、用户生命周期、内容活动与数据决策。自定义面试使用相关性、知识深度、结构表达、沟通与应变五个通用维度。参考答案仅作为评分锚点，不是唯一答案，总分由服务端按权重统一重算。
