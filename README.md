...
面向求职者的 AI 模拟面试工具，支持语音 / 文字作答、AI 自适应追问和按岗位配置的多维评分报告。当前覆盖 8 个岗位：AI 产品经理、产品经理、AGENT 开发工程师、大模型应用开发工程师、前端开发工程师、Java 后端开发工程师、数据分析师、运营。
...
评分维度按岗位区分：产品岗侧重逻辑、产品判断与表达；AI 技术岗侧重技术深度、系统设计和工程实践；前端岗增加前端基础、架构、性能与质量；Java 后端岗增加 Java 基础、系统设计、数据与中间件；数据分析岗侧重 SQL、统计实验、业务洞察和可视化；运营岗侧重增长策略、用户生命周期、内容活动与数据决策。参考答案仅作为评分锚点，不是唯一答案，总分由服务端按岗位权重统一重算。
